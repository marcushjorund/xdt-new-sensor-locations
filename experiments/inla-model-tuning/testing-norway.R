...
plot_inla_model(inla_weighted_norway,  prepared_traffic_links_norway$aadt, test_idx_norway,
                type = "pred_vs_obs",  title = "Weighted Besagproper")

plot_inla_model(inla_model_norway_test, prepared_traffic_links_norway$aadt, test_idx_norway,
                type = "qq",           title = "Besagproper")
plot_inla_model(inla_weighted_norway,   prepared_traffic_links_norway$aadt, test_idx_norway,
                type = "qq",           title = "Weighted Besagproper")

# ── Metrics comparison ────────────────────────────────────────────────────────
pred_besag    <- inla_model_norway_test$predictions$inla_pred[test_idx_norway]
pred_weighted <- inla_weighted_norway$predictions$inla_pred[test_idx_norway]

# Exclude zero-AADT observations from MAPE (division by zero → Inf)
nonzero <- true_aadt_norway > 0

metrics_norway <- data.frame(
  model = c("Besagproper", "Weighted Besagproper"),
  RMSE  = c(.rmse(pred_besag,           true_aadt_norway),         .rmse(pred_weighted,           true_aadt_norway)),
  MAE   = c(.mae( pred_besag,           true_aadt_norway),         .mae( pred_weighted,           true_aadt_norway)),
  MAPE  = c(.mape(pred_besag[nonzero],  true_aadt_norway[nonzero]),.mape(pred_weighted[nonzero],  true_aadt_norway[nonzero]))
)
