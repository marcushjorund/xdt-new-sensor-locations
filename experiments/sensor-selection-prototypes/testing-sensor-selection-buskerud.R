...
inla_weighted_buskerud <- fit_weighted_inla_model(
  data                  = prepared_traffic_links,
...
predictions_total_buskerud <- dplyr::full_join(prepared_traffic_links, inla_weighted_buskerud$predictions)

spatial_hyperpar <- inla_weighted_buskerud$spatial_hyperpar[2:4,"mean"]
tau <- spatial_hyperpar[1]
d   <- spatial_hyperpar[2]
sigma <- spatial_hyperpar[3]

distances <- inla_weighted_buskerud$distances
...
new_sensors_buskerud <- greedy_mi_sensor_selection(data = predictions_total_buskerud, covariance_matrix = prec_and_cov$covariance_matrix_sum, ids = prec_and_cov$ids,
                                                   k = 10, adjacency_matrix = adjacency_matrix, weighting_vars = list(
                                                     functionalRoadClass = setNames(1/seq_len(8), as.character(0:7))))
